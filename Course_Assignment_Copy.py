...
def coursesToCSV(courseList, filename):
    headings = ['Course Name', 'Prof1', 'Prof2', 'Prof3']
    for x in range(4,31):
        headings.append(f'Prof{x}')
    with open(filename, 'w', newline='') as csvfile:
        ...
        for course in courseList:
            # Ensure there are always three columns for professors
            newlist = []
            for prof in course.profsTakingCourse:
                newlist.append(prof.getName())
            professors = newlist + [''] * (30 - len(course.profsTakingCourse))
            row = [course.getName()] + professors[:30]
...
def Course_Assignment():
    ...
    coursesToCSV(CDCList,'sample.csv')
    return CDCList
